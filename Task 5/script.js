...
function filterDogOwers(usersArr) {
  const usersHasDog = usersArr.filter((uObj) => {
    if (uObj.hasDog !== false) return uObj.hasDog;
  });

  console.log("usersHasDog ===", usersHasDog);
}

filterAdults(users);
function filterAdults(userArr) {
  const legalAge = userArr.filter((uObj) => uObj.age >= 18);
